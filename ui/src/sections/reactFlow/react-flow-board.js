...
export default function ReactFlowBoard() {
  ...
  const addNodeToLeft = (id, operation) => {
    setNodes((prevNodes) => {
      ...
      setEdges((prevEdges) => {
        ...
        if (!currentNode) return prevEdges;
        ...
        // Redirect all edges that previously pointed to `id` to now point to newNode
        const incomingEdges = prevEdges.filter((edge) => `${edge.target}` === `${id}`);
        const redirectedIncoming = incomingEdges.map((edge) => ({
          ...edge,
    ...
    setNodes((prevNodes) => {
      ...
      setEdges((prevEdges) => {
        ...
        const newNode = {
          ...
          position: { x: currentNode.position.x + 370, y: currentNode.position.y },
        };

        updatedNodes = [...updatedNodes, newNode];

        // Redirect all edges that previously had `id` as source to now come from newNode
        const outgoingEdges = prevEdges.filter((edge) => `${edge.source}` === `${id}`);
        const redirectedOutgoing = outgoingEdges.map((edge) => ({
          ...edge,
  ...
  const deleteNode = (id, label) => {
    ...
    setNodes((currentNodes) => {
      ...
      // Step 1: Remove deleted node and renumber remaining node IDs
      const filteredNodes = currentNodes.filter(node => Number(node.id) !== deleteId);
      let updatedNodes = filteredNodes.map(node => {
      ...
      setEdges((currentEdges) => {
        // Step 2: Determine prev and next nodes, filter out deleted node's edges
        ...
        // Step 3: If both neighbors exist, bridge them
        ...
        // Step 4: Renumber edge source/target IDs to match renumbered nodes
        ...
        // Step 5: Auto-layout to prevent overlaps
        const { nodes: layoutedNodes, edges: layoutedEdges } = getLayout(updatedNodes, updatedEdges);
        updatedNodes = layoutedNodes;
